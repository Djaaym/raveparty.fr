...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  if (!p)
    return {
      alternates: alternates(`/organisateurs/${params.slug}`, "en"),
      title: "Promoter - RaveRadar",
    };
  ...
  return {
    ...pageMeta({
      ...
      title: seoTitle(`${p.name} - agenda, line-ups & tickets | RaveRadar`),
      description: `${p.name} (${p.city}): ${n > 0 ? `${n} upcoming date${n > 1 ? "s" : ""}` : "past nights"}, line-ups, venues and official ticketing.`,
...
export default function Page({ params }: { params: { slug: string } }) {
  return <PromoterPage lang="en" slug={params.slug} />;
}
